...
output "authorization_server_default_authorize_url" {
  value = format("%s/oauth2/default/v1/authorize", local.spoke_root_url)
}

output "authorization_server_default_jwks_url" {
  value = format("%s/oauth2/default/v1/keys", local.spoke_root_url)
}

output "authorization_server_default_token_url" {
  value = format("%s/oauth2/default/v1/token", local.spoke_root_url)
}

output "authorization_server_default_user_info_url" {
  value = format("%s/oauth2/default/v1/userinfo", local.spoke_root_url)
}